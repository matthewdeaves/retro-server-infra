...
def device_label(ua):
    ...
    A person recognises "This iPhone"; nobody recognises 2001:db8:8f8d:d001.
...
_RELEASE_CACHE = {}
_RELEASE_TTL = 1800          # half an hour; releases are cut by hand, rarely
...
def latest_release(repo, asset_exts=(".dmg",)):
    ...
    with _RELEASE_LOCK:
        if rel:
            ...
            return rel
        # Failed. Keep serving the last good answer, and do not retry for two
        # minutes, so a rate-limited hour is not also a slow one.
        _RELEASE_CACHE[repo] = (now - _RELEASE_TTL + 120, stale)
    return stale
